...
class Visualization(Component):
    def __init__(self):
        ...
        Component.clear_users_data(os.path.join(os.path.dirname(os.path.abspath(__file__)),"templates"))
    ...
    def __graphs_selection(self):
        ...
        for name,html in Allhtmls:
            target = '<div><a class="w3-button w3-dark-grey" href='+html+'>'+name+'<i class="fa fa-arrow-right"></i></a></div>'
    ...
    def __graphrendering(self,graphname:str):
        TARGET = str(self.get_user_ip())+"/Temp_" + graphname.replace('<','').replace('>','') + "_0.html"
