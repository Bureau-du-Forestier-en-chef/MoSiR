...
class Download(Component):
    ...
    def __get_graphs(self):
        HTMLsandnames = []
        for graphfile in self._get_graphs_files() + self._get_inputs_files() + self._get_reporting_files() + self._get_results_files():
    ...
    def can_view(self):
        return (len(self._get_graphs_files()) > 0 ) or  (len(self._get_inputs_files()) > 0) or (len(self._get_reporting_files()) > 0)
